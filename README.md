...
```bash
filecrawler v0.1.4 by Helvio Junior
Active Directory, BloodHound, NTDS hashes and Password Cracks correlation tool
https://github.com/helviojunior/filecrawler
    
 [+] Startup parameters
     command line: filecrawler --stats 
     module: stats
...
```bash
pip3 install --upgrade filecrawler
```
...
```bash
filecrawler --create-db
```


## Importing BloodHound files

We can import all full BloodHound files into filecrawler, correlate data, and sync it to Neo4J BloodHound Database. So you can use only filecrawler to import JSON files directly into Neo4j database instead of use `extremely slow BloodHound User Interface`

```bash
# Bloodhound ZIP File
filecrawler --bloodhound --import-data ~/Desktop/client.zip

# Bloodhound JSON File
filecrawler --bloodhound --import-data ~/Desktop/20220912105336_users.json
```

**Note:** The filecrawler is capable to import BloodHound ZIP File and JSON files, but we recommend to use ZIP file, because the filecrawler will automatically order the files to better data correlation. 

### Sync data to Neo4j BloodHound database

```bash
# Bloodhound ZIP File
filecrawler --bloodhound --sync 10.10.10.10:7687 -d neo4j -u neo4j -p 12345678
```

**Note:** The filecrawler implementation of bloodhount-importer was inpired from [Fox-It BloodHound Import](https://github.com/fox-it/bloodhound-import) implementation. We implemented several changes to save all data in filecrawler SQLite database and after that do an incremental sync to Neo4J database. With this strategy we have several benefits such as at least 10x faster them original BloodHound User interface.
...
```bash
filecrawler --secrets-dump -target LOCAL -ntds ~/Desktop/ntds.dit -system ~/Desktop/SYSTEM
```
...
```bash
filecrawler --ntlm-hash --import-ntds ~/Desktop/client_name.ntds
```
...
```bash
filecrawler --ntlm-hash --company clientCompanyName --import-cracked ~/Desktop/cracked.txt
```
...
```bash
filecrawler --bloodhound --mark-owned 10.10.10.10 -d neo4j -u neo4j -p 123456
```
