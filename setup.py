...
meta=$(cat filecrawler/__meta__.py | sed "s/__build__.*/__build__ = "${build}"/")
echo "$meta" > filecrawler/__meta__.py

git add ./filecrawler/__meta__.py
...
with open(f"{here}/filecrawler/__meta__.py") as f:
    exec(f.read(), meta)
...
package_data.update(
    {
        ...
        for dp, dn, filenames in os.walk(f"{here}/filecrawler") for f in filenames
...
setup(
    ...
    entry_points={'console_scripts': [
        'filecrawler=filecrawler.filecrawler:run',
        ]
    },
    project_urls={
